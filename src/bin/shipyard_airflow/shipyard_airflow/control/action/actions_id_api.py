...
class ActionsIdResource(BaseResource):
    ...
    def on_get(self, req, resp, **kwargs):
        ...
        resp.text = self.to_json(
            self.get_action(action_id=kwargs['action_id'],
    ...
    def get_action(self, action_id, verbosity):
        ...
        dag = self.get_dag_runs_by_id(dag_id, dag_execution_date)
        ...
        notes = notes_helper.get_action_notes(action_id=action_id,
                                              verbosity=verbosity)
        action['notes'] = []
        for note in notes:
    ...
    def get_dag_runs_by_id(self, dag_id, execution_date):
        ...
        """
        dag_run_list = self.get_dag_run_api(dag_id, execution_date)
    ...
    def get_validations_db(self, action_id):
        ...
        """
        return SHIPYARD_DB.get_validation_by_action_id(action_id=action_id)
    ...
    def get_dag_run_api(self, dag_id, execution_date):
        """
        Retrieves a dag_run using the Airflow v2 REST API.
        Returns a list of dictionaries with
        the same structure as get_dag_run_db().
        """
        return AIRFLOW_API.get_dag_runs_by_id(
